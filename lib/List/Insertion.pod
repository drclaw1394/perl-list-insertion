...
No object methods for comparison. Just basic C<E<<=>>,C<E<>=>, C<le>  and C<ge>
operators are used for string and numeric comparison.

=item List element are homogeneous (no undefined items)

The items in the list are expected to have have the same (complex) data
structure. This can be a simple scalar like a or and array of arrays or hashes
to any level of complexity. They can also be a object (a reference). It doesn't
matter, as long as all the items are of the same structure.

=back

Although intended for searching of insertion points, this module can also be
used for general searching of elements with a simple check of equality between
the search key and  value at the found index. 
